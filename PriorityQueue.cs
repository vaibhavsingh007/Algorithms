...
namespace Algorithms
{
    ...
    public class PriorityQueue<TPriority, TValue> : ICollection<KeyValuePair<TPriority, TValue>>
    {
        ...
        public PriorityQueue(IEnumerable<KeyValuePair<TPriority, TValue>> data, IComparer<TPriority> comparer)
        {
            ...
            _valueComparer = Comparer<TValue>.Default;
            _comparer = comparer;
        ...
        /// <summary>
        /// Update keys if new value is less than old
        /// </summary>
        /// <param name="updateKey"></param>
        public void DecreaseKey(KeyValuePair<TPriority, TValue> updateKey)
        {
            int i = _baseHeap.IndexOf(_baseHeap.Find(b => _valueComparer.Compare(b.Value, updateKey.Value) == 0));
            if ((_valueComparer.Compare(_baseHeap[i].Value, updateKey.Value) == 0) &&
                _comparer.Compare(_baseHeap[i].Key, updateKey.Key) > 0)
            {
                _baseHeap[i] = updateKey;
                // heapify data
                for (int pos = _baseHeap.Count / 2 - 1; pos >= 0; pos--)
                    HeapifyFromBeginningToEnd(pos);
            }
